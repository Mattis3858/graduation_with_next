...
const callSensoryApi = async (data) => {
  try {
      ...
      const response = await fetch('https://7a8f-140-119-19-30.ngrok-free.app/SensoryAI/prediction', {
        method: 'POST',
        headers: {
  ...
  const handleFlavorDataSubmit = (flavorData) => {
    console.log('FindGoodTea - handleFlavorDataSubmit is called with data:', flavorData); // 确保这行代码被执行了并且收到了正确的数据
    ...
    console.log('FindGoodTea - valuesObject should be updated:', newValuesObject); // 打印出新的 state 值，确认它是否正确
  };

  // user click step_button, go to next step
  const handleNext = () => {
    if (activeStep === 2) { //一定要是2! 請務必注意Step 順序
  ...
  const getStepLabel = (index) => {
    if (index === 1) {
      if (selectedOption === 0) { //choose"開始探索風味" 
        return '參考前次結果';
      } else if (selectedOption === 1) { //choose"一邊品味茶葉一邊探索風味"
        return '學習品茶步驟';
      }
    }
    return steps[index];
  ...
  const renderStepContent = (step) => {
    switch (step) {
      ...
      case 2: // send posttest data to flavor.js and return the current flavor data in flavor.js
        return <Flavor ref={flavorRef} postData={posttestData} onFlavorDataSubmit={handleFlavorDataSubmit} />;
      case 3:
        return selectedOption === 0 ? (
